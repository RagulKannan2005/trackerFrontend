...
public class TrackerSectionController {
    ...
    @PreAuthorize("hasRole('ADMIN')")
    @DeleteMapping("/tracker/{trackerId}/section/{sectionId}")
    public ResponseEntity<Void> removeSectionByTrackerAndSection(@PathVariable Long trackerId, @PathVariable Long sectionId) {
        trackerSectionService.removeSectionByTrackerAndSection(trackerId, sectionId);
        return ResponseEntity.noContent().build();
    }
}
